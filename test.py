...
if __name__ == '__main__':
	...
	for archPrograma in sorted(glob.glob('programas'+os.sep+'*.py')):
		...
		for archEntrada in sorted(glob.glob('entradas'+os.sep+'programa*entrada*.txt')):
			numProgramaDeEntrada = archEntrada[len(archPrograma) - 5]
			if numPrograma != numProgramaDeEntrada:
				continue
